...
# ---------------------------------------------------------------------------
# Optional PyBaMM backend for battery simulation
# ---------------------------------------------------------------------------
try:
    import pybamm
    HAS_PYBAMM = True
    logging.getLogger(__name__).debug("PyBaMM is available – SPM/DFN backend enabled")
except ImportError:
    HAS_PYBAMM = False
    logging.getLogger(__name__).debug("PyBaMM not found – falling back to built-in models")
...
def _simulate_discharge(
    ...
    if HAS_PYBAMM and config.chemistry in ["LiFePO4", "LiCoO2", "graphite"]:
        logger.info(f"Using PyBaMM backend for {config.chemistry} simulation")
        try:
            result = simulate_pybamm(config)
            
            soc_used = np.array(result["soc"])
            V_used = np.array(result["voltage"])
            t_array = np.array(result["time_min"])
            Q_delivered = np.array(result["capacity_mAh"])[-1] if len(result["capacity_mAh"]) > 0 else 0
            end_idx = len(soc_used)
            
            perf.delivered_capacity_mAh = Q_delivered
            perf.utilization_pct = (Q_delivered / Q) * 100 if Q > 0 else 0
            
            cap_array = np.array(result["capacity_mAh"])
            ...
            logger.warning(f"PyBaMM simulation failed ({e}), falling back to built-in solver")
...
def simulate_pybamm(config: BatteryConfig, protocol: str = 'discharge', t_eval_hours: float = None) -> dict:
    """
    Run battery simulation using PyBaMM.
    Uses Single Particle Model (SPM) by default.
    """
    if not HAS_PYBAMM:
    ...
    # Choose parameters based on chemistry
    if config.chemistry == "LiFePO4":
        param = pybamm.ParameterValues("Prada2013")
    elif config.chemistry == "LiCoO2":
        param = pybamm.ParameterValues("Marquis2019")
    else:
        param = pybamm.ParameterValues("Chen2020") # default NMC
        
    # Override parameters with config
    A_m2 = config.electrode_area_cm2 * 1e-4
    
    try:
        # Try to set some basic overrides without breaking the parameter set
        param.update({
            "Electrode height [m]": np.sqrt(A_m2),
            "Electrode width [m]": np.sqrt(A_m2),
            "Negative electrode thickness [m]": config.anode_thickness_um * 1e-6,
            "Positive electrode thickness [m]": config.cathode_thickness_um * 1e-6,
            "Separator thickness [m]": config.separator_thickness_um * 1e-6,
            "Lower voltage cut-off [V]": config.cutoff_V,
            "Upper voltage cut-off [V]": config.max_V,
        }, check_already_exists=False)
    except Exception as e:
        logger.warning(f"Could not apply some PyBaMM overrides: {e}")

    # Set C-rate
    param.update({
        "Current function [A]": "[current data] " + str(config.C_rate) + " * C",
    }, check_already_exists=False)

    # Use casadi solver (more robust on Windows)
    solver = pybamm.CasadiSolver(mode="safe", atol=1e-6, rtol=1e-6)
    
    # Run simulation
    sim = pybamm.Simulation(model, parameter_values=param, solver=solver)
    
    # Default 1 hour simulation if not provided
    if t_eval_hours is None:
        t_eval_hours = 1.0 / config.C_rate if config.C_rate > 0 else 1.0
        # Add 10% margin
        t_eval_hours *= 1.1
        
    t_eval = np.linspace(0, t_eval_hours * 3600, 500)
    ...
    try:
        ...
        soc = 1.0 - (solution["Discharge capacity [A.h]"].entries / 
                     param["Nominal cell capacity [A.h]"])

    return {
        "time_min": (t_s / 60).tolist(),
        "voltage": V.tolist(),
        "current": I.tolist(),
        "soc": soc.tolist(),
        "capacity_mAh": (solution["Discharge capacity [A.h]"].entries * 1000).tolist()
    }
