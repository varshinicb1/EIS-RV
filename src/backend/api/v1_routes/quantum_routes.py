...
async def calculate_band_gap(request: BandGapRequest):
    ...
    **Example Response:**
    ...
    try:
        ...
        # Estimate HOMO using Koopmans' theorem heuristic when ALCHEMI is
        # unavailable.  The reference vacuum level for a molecule near an
        # electrode is approximately -4.5 eV (vs. absolute vacuum scale),
        # and HOMO ≈ -(IE) where IE ≈ 4.5 + band_gap/2 gives a reasonable
        # first-order estimate for closed-shell molecules.
        homo = -(4.5 + band_gap / 2.0)
        lumo = homo + band_gap
        ...
        return QuantumResponse(
            ...
            data={
                ...
                "homo_eV": homo,
                "lumo_eV": lumo,
                "homo_lumo_method": request.method if not engine.placeholder_mode else "Koopmans-heuristic (placeholder)"
            },
            metadata={
                "smiles": request.smiles,
                "method": request.method,
