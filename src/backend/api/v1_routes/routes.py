...
class PredictRequest(BaseModel):
    ...
    material_id: Optional[str] = Field(None, description="Legacy/Alternative material identifier")
    thickness_um: Optional[float] = Field(None, description="Electrode thickness (µm)")
    porosity: Optional[float] = Field(None, description="Electrode porosity")
    temperature_c: Optional[float] = Field(None, description="Temperature (°C)")
    frequency_range: Optional[List[float]] = Field(None, description="Frequency range [min, max] (Hz)")
    num_points: Optional[int] = Field(None, description="Number of frequency points")
...
class KKValidateRequest(BaseModel):
    ...
    Z_real: Optional[List[float]] = Field(None, max_length=100000, description="Real impedance (Ω)")
    Z_imag: Optional[List[float]] = Field(None, max_length=100000, description="Imaginary impedance (Ω)")
...
async def predict_material(request: PredictRequest):
    ...
    """
    try:
        comp_dict = request.composition
        if not comp_dict and request.material_id:
            comp_dict = {request.material_id: 1.0}
        if not comp_dict:
            comp_dict = {"graphene": 1.0}  # default fallback
        comp = MaterialComposition(components=comp_dict)
...
async def validate_kk(request: KKValidateRequest):
    ...
    try:
        ...
        Z_real = np.array(request.Z_real if request.Z_real is not None else (request.z_real or []))
        Z_imag = np.array(request.Z_imag if request.Z_imag is not None else (request.z_imag or []))
...
async def validate_perovskite(max_spectra: int = 10):
    ...
    """
    try:
        from src.backend.core.engines.validation import validate_against_perovskites
        ...
        raise HTTPException(
            status_code=404,
            detail="Perovskite dataset not found. Place eis_perovskites.csv in data/datasets/external/",
        )
    except Exception as e:
...
async def get_pipeline_stats():
    ...
    try:
        ...
        from ...research.schema import get_connection
...
async def search_pipeline(request: PipelineSearchRequest):
    ...
    try:
        from ...research.config import DB_PATH
        from ...research.schema import get_connection
        from ...research.search import DatasetSearch
...
class CVSimRequest(BaseModel):
    ...
    active_mass_mg: Optional[float] = Field(None, description="Active material mass (mg) for capacitance calculation")
...
async def simulate_cv_endpoint(request: CVSimRequest):
    ...
    try:
        ...
        # Calculate specific capacitance if mass is provided
        if hasattr(request, 'active_mass_mg') and request.active_mass_mg is not None and request.active_mass_mg > 0:
            try:
                from src.backend.core.capacitance_calculator import get_capacitance_calculator
                calculator = get_capacitance_calculator()
                
                cap_result = calculator.from_cv_data(
                    potential=result.E.tolist(),
                    current=result.i_total.tolist(),
                    scan_rate_mV_s=request.scan_rate_V_s * 1000,
                    mass_g=request.active_mass_mg / 1000,
                    area_cm2=request.electrode_area_cm2,
                )
                
                result.specific_capacitance_F_g = cap_result.specific_capacitance_F_g
                result.areal_capacitance_F_cm2 = cap_result.areal_capacitance_F_cm2
            except Exception as e:
                logger.warning(f"Capacitance calculation failed: {e}")
...
async def estimate_cost(request: CostEstimateRequest):
    """Estimate material cost for a given composition and mass or material list."""
    total_cost = 0.0
    breakdown = []
    ...
    if request.materials:
        total_mass = sum(float(m.get("quantity_g", 0) or 0) for m in request.materials)
        if total_mass > 0:
            mass_g = total_mass
            composition = {m.get("name"): float(m.get("quantity_g", 0) or 0) / total_mass for m in request.materials if m.get("name")}
        else:
            composition = {m.get("name"): 1.0 for m in request.materials if m.get("name")}
...
async def get_external_material_data(formula: str):
    ...
    """
    try:
        from src.backend.core.engines.external_data import (
