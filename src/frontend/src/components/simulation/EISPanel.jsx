...
export default function EISPanel() {
  ...
  const runSimulation = useCallback(async () => {
    setRunning(true);
    try {
      const res = await fetch('/api/v2/eis', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        console.warn('EIS backend returned', res.status, '— using local solver');
        setResult(computeEIS(params));
      }
    } catch (err) {
      console.warn('EIS backend unreachable — using local solver:', err.message);
      setResult(computeEIS(params));
    } finally {
      setRunning(false);
    }
