...
export default function DRTPanel() {
  ...
  useEffect(() => {
    ...
    // WHITE BACKGROUND - Scientific publication style
    ...
    grad.addColorStop(0, 'rgba(37,99,235,0.3)'); grad.addColorStop(1, 'rgba(37,99,235,0.02)');
    ...
    // Peak markers - scientific colors
    const PROC_COLORS = { charge_transfer: '#DC2626', diffusion: '#EA580C', double_layer: '#059669', adsorption: '#7C3AED', unknown: '#6B7280' };
    (peaks || []).forEach(p => {
      const px = toX(Math.log10(p.tau)); const py = toY(p.gamma);
      ctx.beginPath(); ctx.arc(px, py, 5, 0, Math.PI * 2);
      ctx.fillStyle = PROC_COLORS[p.process] || '#6B7280'; ctx.fill();
      ctx.strokeStyle = '#FFFFFF'; ctx.lineWidth = 1; ctx.stroke();
      ctx.font = '10px "Times New Roman", Times, serif'; ctx.fillStyle = '#111827'; ctx.textAlign = 'center';
      ctx.fillText(`${p.process}`, px, py - 10);
      ctx.fillText(`τ=${p.tau.toExponential(1)}s`, px, py - 20);
    });

    // Labels - Times New Roman
    ctx.font = '12px "Times New Roman", Times, serif'; ctx.fillStyle = '#374151'; ctx.textAlign = 'center';
    for (let x = Math.ceil(xMin); x <= Math.floor(xMax); x++) {
    ...
    // Title
    ctx.font = 'bold 12px "Times New Roman", Times, serif'; ctx.fillStyle = '#111827'; ctx.textAlign = 'left';
    ctx.fillText('Distribution of Relaxation Times', pad.l, 16);
  }, [result]);

  const procColor = { charge_transfer: '#DC2626', diffusion: '#EA580C', double_layer: '#059669', adsorption: '#7C3AED', unknown: '#6B7280' };
            ...
            {(result.peaks || []).length === 0 ? (
              ...
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', borderBottom: '1px solid var(--border-default)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: procColor[p.process] || '#6B7280' }} />
