...
function CVPlot({ data }) {
  ...
  useEffect(() => {
    ...
    if (data.i_pa != null) {
      const peaks = [
        { e: data.E_pa, i: data.i_pa, color: '#DC2626', label: 'ipa' },
        { e: data.E_pc, i: data.i_pc, color: '#059669', label: 'ipc' }
      ];
      peaks.forEach(p => {
  ...
  return <canvas ref={canvasRef} style={{ width: '100%', height: '100%', background: '#FFFFFF' }} />;
}
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.warn('CV backend returned', res.status, '— using local solver');
        setResult({ ...simulateCV(params), engine: 'V_EMULATOR_LITE' });
        setRunning(false);
      }
    } catch (err) {
      console.warn('CV backend unreachable — using local solver:', err.message);
      setResult({ ...simulateCV(params), engine: 'V_EMULATOR_LITE' });
      setRunning(false);
    }
  }, [params]);
