...
function GCDPlot({ data }) {
  ...
  useEffect(() => {
    ...
    // Ticks
    ctx.fillStyle = '#6B7280'; ctx.font = '10px "Times New Roman", serif';
    ...
    ctx.fillStyle = '#374151'; ctx.font = '11px "Times New Roman", serif';
...
export default function GCDPanel() {
  ...
  const simulate = useCallback(async () => {
    setComputing(true);
    try {
      const res = await fetch('/api/v2/gcd', {
      ...
      if (res.ok) {
        ...
        console.warn('GCD backend returned', res.status, '— using local solver');
        setResult(simulateGCDLocal(params));
      }
    } catch (err) {
      console.warn('GCD backend unreachable — using local solver:', err.message);
      setResult(simulateGCDLocal(params));
    } finally {
      setComputing(false);
    }
