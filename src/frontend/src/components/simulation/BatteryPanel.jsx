...
export default function BatteryPanel() {
  ...
  const simulate = useCallback(async () => {
    setComputing(true);
    try {
      const res = await fetch('/api/v2/battery', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        setResult({ ...data, engine: data.engine || 'SENTINEL_CORE' });
      } else {
        console.warn('Battery backend returned', res.status, '— using local solver');
        setResult(simulateBatteryLocal(params));
      }
    } catch (err) {
      console.warn('Battery backend unreachable — using local solver:', err.message);
      setResult(simulateBatteryLocal(params));
    } finally {
      setComputing(false);
    }
