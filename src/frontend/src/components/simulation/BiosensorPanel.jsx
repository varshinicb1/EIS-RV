...
function ElectrodeViewer3D({ geometry, coating }) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: 'linear-gradient(to bottom, #F9FAFB 0%, #E5E7EB 100%)', cursor: 'grab' }}>
      <Canvas camera={{ position: [0, 5, 9], fov: 40 }} gl={{ antialias: true, alpha: false, powerPreference: 'high-performance' }} dpr={[1, 2]}>
        ...
        <directionalLight position={[8, 12, 6]} intensity={1.5} castShadow shadow-mapSize-width={2048} shadow-mapSize-height={2048} />
        <directionalLight position={[-5, 8, -4]} intensity={0.8} color="#F3F4F6" />
        <pointLight position={[0, 6, 0]} intensity={0.5} color="#FFFFFF" />
        {/* Scale grid for research reference - light gray for white background */}
        <Grid position={[0, -0.21, 0]} args={[20, 20]} cellSize={1} cellThickness={0.3} cellColor="#D1D5DB" sectionSize={5} sectionThickness={0.8} sectionColor="#9CA3AF" fadeDistance={25} />
...
export default function BiosensorPanel() {
  ...
  useEffect(() => {
    fetch('/api/v2/biosensor/library')
  ...
  const runSimulation = async () => {
    ...
    try {
      const res = await fetch('/api/v2/biosensor/simulate', {
  ...
  const optimizeMaterial = async () => {
    ...
    try {
      const res = await fetch('/api/v2/biosensor/optimize', {
        method: 'POST', headers: {'Content-Type': 'application/json'},
