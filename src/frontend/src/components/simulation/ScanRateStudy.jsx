    ...
    const results = await Promise.all(rates.map(async (rate) => {
      try {
        const res = await fetch('/api/v2/cv', {
          ...
          body: JSON.stringify({ ...p, scan_rate: rate }),
        });
        if (res.ok) {
          const data = await res.json();
          const iArr = data.i_total || [];
          const eArr = data.E || [];
          const maxIdx = iArr.reduce((mi, v, i) => v > iArr[mi] ? i : mi, 0);
          const minIdx = iArr.reduce((mi, v, i) => v < iArr[mi] ? i : mi, 0);
          const ipa = Math.max(0, iArr[maxIdx] ?? 0);
          const ipc = Math.abs(Math.min(0, iArr[minIdx] ?? 0));
          const Epa = eArr[maxIdx] ?? 0;
          const Epc = eArr[minIdx] ?? 0;
          const ip_rs = 0.4463 * Math.pow(p.n_electrons, 1.5) * F * (p.area || 0.0707) *
            (p.C_ox * 1e-3) * Math.sqrt((p.D_ox || 7.6e-6) * rate * F / (R * T));
          return { E: eArr, I: iArr, ipa, ipc, Epa, Epc, dEp: Math.abs(Epa - Epc), ip_rs, rate };
        }
      } catch {}
      return simulateCVAtRate(p, rate);
    }));
    setStudies(results);
  }, [rates, p]);
