...
from vanl.research_pipeline.config import (
...
Examples:
    ...
    python -m vanl.research_pipeline.scheduler --mode once

    # Run once with more papers
    python -m vanl.research_pipeline.scheduler --mode once --max-per-query 20

    # Run continuous loop (every 24h)
    python -m vanl.research_pipeline.scheduler --mode loop --interval 24

    # Quick test with 5 papers per query
    python -m vanl.research_pipeline.scheduler --mode once --max-per-query 5 -v
        """
    )
